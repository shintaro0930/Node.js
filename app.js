...
const login_page = fs.readFileSync('./lodin.ejs', 'utf8');

const max_num = 10;
const file_name = 'mydata.txt';
var message_data;
...
function getFromClient(request, response) {
    ...
    switch (url_parts.pathname) {
        case '/':
            reponse_index(request, response);
            break;

        case '/login':
            reponse_login(request, response);
            break;
...
function reponse_login(request, response) {
    var content = ejs.render(login_page, {});
    response.writeHead(200, { 'Content-Type': 'text/html' });
    reponse.write(content);
    response.end();
}
